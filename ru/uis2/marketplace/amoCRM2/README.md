...
После добавления учетных данных активируется кнопка "Проверить соединение с аккаунтом CRM". При нажатии на кнопку будет выведен статус заведенных учетных данных: <br /> 
- если соединение успешно, появится сообщение: "Синхронизация прошла успешно". <br />
- если возникла ошибка, отобразится уведомление: "Ошибка соединения".<br />

Если выдалась "Ошибка соединения" - нужно обновить учетные данные: <br />
- либо переподключить нажав на кнопку обновления;
- либо удалить учетные данные и завести их заново.   <br />
...
4. Нажмите кнопку **Синхронизировать сотрудников** для ручной синхронизации сотрудников. <br />
...
1. **Первичные обращения** - выберите требуемый способ обработки чатов по первичным обращениям, т.е. когда контакт в amoCRM еще не найден.    <br /> 
<br /> 

2. **Повторные обращения** - выберите требуемый способ обработки чатов по повторным обращениям, т.е. когда контакт в amoCRM уже существует.    <br />

В случае выбранного варианта "Создавать сделку, если нет активных" становится доступной дополнительная настройка с указание времени с момента закрытия последней активной сделки.  <br />
<br />


3. **Создавать в момент**: 
- Начала чата - создаем сущности, если этого требуют остальные настройки, в начале чата. Выбрана по умолчанию. 
- Принятия чата в работу - настройка, при которой входящие обращения будут провоцировать создание сущностей, передачу доп параметров и теги в amoCRM только когда чат будет принят в работу, и будет отправлено/получено сообщение, при этом не важно: первое сообщение после принятия чата будет от оператора или клиента, мы создадим сделку и/или контакт в amoCRM.
<br /> 
<br />

4. **Назначать на сотрудника**:
- Ответственного из настроек интеграции - ответственным становится сотрудник, который выбран во вкладке "Ответственные" в зависимости от заданных условий. <br /> 
- Принявшего чат - синхронизированный сотрудник, который принял чат, будет назначаться ответственным за сущность в CRM. Если сотрудник не синхронизирован - назначаем ответственного по логике “Ответственного из настроек интеграции”. <br /> 
<br /> 

5. **Обновлять ответственного при повторных обращениях** - при выбранной настройке обновляем ответственного в существующих сущностях и при повторных обращениях.<br /> 

<br /> 

6. **Передавать групповые чаты** - при выбранной настройке создаем по групповым чатам сущности из п.1 в amoCRM. При этом контактные данные в самом контакте не фиксируем, передаем только название группы и в сообщениях от каждого посетителя фиксируем его номер телефона или никнейм. <br /> 

<br />  

7. **Фильтрация чатов** - задайте условия, если требуется фильтровать чаты по сайтам, рекламным кампаниям и проч.<br /> 
...
8. **Создавать задачу** - выберите настройку, если требуется создание задач по чатам. При включении настройки, появляются дополнительные поля: <br />  
...
9. **Проставлять теги**  - при выключенной настройке теги не будут ставиться на сущности amoCRM. И также, скрыто поле "Список тегов". <br />

При включении настройки - активируется поле **Список тегов**. <br />  
Есть возможность выбрать тег как из фиксированного списка, так и добавить пользовательские теги, при нажатии на "Добавить значение". <br /> 
При выборе тегов, проставляться они будут на сущности контакта и сделки, при их создании.
