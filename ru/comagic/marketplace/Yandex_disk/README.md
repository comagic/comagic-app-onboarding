...
При её выборе появится дополнительная настройка со списком тегов, в нем необходимо отметить все теги, которые требуется передавать в название файла.  <br />
    ...
    - **Через какое время проверять простановку тегов (мин)** - время, через которое запускаем процесс передачи данных по звонку в ЯД , после получения события простановки тега из нашего ЛК. Требуется в случаях, если присутствует разрыв между простановкой нескольких требуемых тегов , участвующих в отложенной фильтрации.   <br />
